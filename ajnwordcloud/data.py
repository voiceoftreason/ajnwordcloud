...
def load_txt(file, minwordlength=3, extra_stopwords=[]):
    """
    Load text file and count words
    ...
    with open(file,"r+",encoding='utf8') as f:
        text = f.read().lower()
        wordcount = count_words(text, minwordlength=minwordlength, extra_stopwords=extra_stopwords)
    return wordcount

def count_words(text, minwordlength=3, extra_stopwords=[]):
    """
    Process text input
    
    Arguments:
    text -- text block to count
    ...
    for word in text.split():
        word = word.strip()
        if word not in stoplist and word not in extra_stopwords and len(word) >= minwordlength:
